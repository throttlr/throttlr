...
import throttlr.twitter.internal.TimeSource;
...
public class CruiseThrottleStrategy implements ThrottleStrategy {
    ...
    @Override
    public <T extends TwitterResponse> T throttle(String endpoint, Supplier<Either<TwitterException, T>> supplier) throws TwitterException {
        synchronized (rateLimits) {
            try {
                RateLimitStatus rateLimitStatus = rateLimits.get(endpoint);
                ...
                if (rateLimitStatus != null && now < rateLimitStatus.getResetTimeInSeconds()) {
                    ...
                    if (0 < rateLimitStatus.getRemaining()) {
                        ...
                        logger.debug("Cruise wait {} milliseconds with {} remains for {}", timeout, rateLimitStatus.getRemaining(), endpoint);
                        rateLimits.wait(timeout);
                    } else {
                        long timeout = 1000L * (reset - now);
                        logger.debug("Cruise wait {} milliseconds by no remains for {}", timeout, endpoint);
                        rateLimits.wait(timeout);
                    }
                }
                Either<TwitterException, T> either = supplier.get();
                if (either.isRight()) {
                    rateLimits.put(endpoint, either.getRight().getRateLimitStatus());
